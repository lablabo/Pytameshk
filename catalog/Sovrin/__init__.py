import asyncio
import json
import pprint

from indy import pool, ledger, wallet, did
from indy.error import IndyError


class controller:


    pool_name = 'pool'
    genesis_file_path = '/home/mohammad/mohammad/indy-sdk/cli/docker_pool_transactions_genesis'
    wallet_config = json.dumps({"id": "wallet"})
    wallet_credentials = json.dumps({"key": "wallet_key"})
    # Set protocol version to 2 to work with the current version of Indy Node
    PROTOCOL_VERSION = 2

    def __init__(self, action, libraries):
        self.main()

    def print_log(self, value_color="", value_noncolor=""):
        HEADER = '\033[92m'
        ENDC = '\033[0m'
        print(HEADER + value_color + ENDC + str(value_noncolor))

    async def write_nym_and_query_verkey(self):
        try:
            await pool.set_protocol_version(self.PROTOCOL_VERSION)
            pool_config = json.dumps({'genesis_txn': self.genesis_file_path})
            self.print_log('\n1. Create new pool ledger configuration to connect to ledger.\n')
            await pool.create_pool_ledger_config(config_name=self.pool_name, config=pool_config)
            try:
                # await pool.delete_pool_ledger_config(config_name=pool_name)
                await pool.create_pool_ledger_config(config_name=self.pool_name, config=pool_config)
            except IndyError:
                await pool.delete_pool_ledger_config(config_name=self.pool_name)
                await pool.create_pool_ledger_config(config_name=self.pool_name, config=pool_config)

            self.print_log('\n2. Open ledger and get handle\n')
            pool_handle = await pool.open_pool_ledger(config_name=self.pool_name, config=None)

            self.print_log('\n3. Create new identity wallet\n')
            await wallet.create_wallet(self.wallet_config, self.wallet_credentials)

            self.print_log('\n4. Open identity wallet and get handle\n')
            wallet_handle = await wallet.open_wallet(self.wallet_config, self.wallet_credentials)


            self.print_log('\n5. Generate and store steward DID and verkey\n')

            steward_seed = '000000000000000000000000Steward1'
            did_json = json.dumps({'seed': steward_seed})
            steward_did, steward_verkey = await did.create_and_store_my_did(wallet_handle, did_json)
            self.print_log('Steward DID: ', steward_did)
            self.print_log('Steward Verkey: ', steward_verkey)

            self.print_log('\n6. Generating and storing trust anchor DID and verkey\n')
            trust_anchor_did, trust_anchor_verkey = await did.create_and_store_my_did(wallet_handle, "{}")
            self.print_log('Trust anchor DID: ', trust_anchor_did)
            self.print_log('Trust anchor Verkey: ', trust_anchor_verkey)

            self.print_log('\n7. Building NYM request to add Trust Anchor to the ledger\n')
            nym_transaction_request = await ledger.build_nym_request(submitter_did=steward_did,
                                                                     target_did=trust_anchor_did,
                                                                     ver_key=trust_anchor_verkey,
                                                                     alias=None,
                                                                     role='TRUST_ANCHOR')
            self.print_log('NYM transaction request: ')
            pprint.pprint(json.loads(nym_transaction_request))

            self.print_log('\n8. Sending NYM request to the ledger\n')
            nym_transaction_response = await ledger.sign_and_submit_request(pool_handle=pool_handle,
                                                                            wallet_handle=wallet_handle,
                                                                            submitter_did=steward_did,
                                                                            request_json=nym_transaction_request)
            self.print_log('NYM transaction response: ')
            pprint.pprint(json.loads(nym_transaction_response))

            self.print_log('\n9. Generating and storing DID and verkey representing a Client '
                      'that wants to obtain Trust Anchor Verkey\n')

            client_did, client_verkey = await did.create_and_store_my_did(wallet_handle, "{}")
            self.print_log('Client DID: ', client_did)
            self.print_log('Client Verkey: ', client_verkey)

            self.print_log('\n10. Building the GET_NYM request to query trust anchor verkey\n')
            get_nym_request = await ledger.build_get_nym_request(submitter_did=client_did,
                                                                 target_did=trust_anchor_did)
            self.print_log('GET_NYM request: ')
            pprint.pprint(json.loads(get_nym_request))

            self.print_log('\n11. Sending the Get NYM request to the ledger\n')
            get_nym_response_json = await ledger.submit_request(pool_handle=pool_handle,
                                                               request_json=get_nym_request)
            get_nym_response = json.loads(get_nym_response_json)
            self.print_log('GET_NYM response: ')
            pprint.pprint(get_nym_response)

            # See whether we received the same info that we wrote the ledger in step 4.
            self.print_log('\n12. Comparing Trust Anchor verkey as written by Steward and as retrieved in GET_NYM '
                      'response submitted by Client\n')

            self.print_log('Written by Steward: ', trust_anchor_verkey)
            verkey_from_ledger = json.loads(get_nym_response['result']['data'])['verkey']
            self.print_log('Queried from ledger: ', verkey_from_ledger)
            self.print_log('Matching: ', verkey_from_ledger == trust_anchor_verkey)

            # Do some cleanup.
            self.print_log('\n13. Closing wallet and pool\n')
            await wallet.close_wallet(wallet_handle)
            await pool.close_pool_ledger(pool_handle)

            self.print_log('\n14. Deleting created wallet\n')
            await wallet.delete_wallet(self.wallet_config, self.wallet_credentials)

            self.print_log('\n15. Deleting pool ledger config\n')
            await pool.delete_pool_ledger_config(self.pool_name)


        except IndyError as e:
            print('Error occurred: %s' % e)

    def main(self):
        loop = asyncio.get_event_loop()
        loop.run_until_complete(self.write_nym_and_query_verkey())
        loop.close()


